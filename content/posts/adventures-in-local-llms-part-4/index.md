...
For this, I used paperless-gpt (AI-assisted document ingest which extracts and applies metadata to the documents) as my test. The task is well-defined enough that differences between models become apparent quickly: extract the correspondent, document date, document type, title, and tags from a PDF.
...
This was the hardest to evaluate. Coding assistants need to understand code structure, follow instructions precisely, remember details over long sessions, and generate working code — all while being interactive enough to iterate with you. Local models on consumer hardware were not great at this yet. But I still tried to assess:
...
Long story short, after experimenting for a while, I decided to upgrade my GPU to an **RTX 5060 Ti with 16GB of VRAM**. This wasn't a random choice — at the current time, it's the cheapest consumer card that gave me modern CUDA architecture and 16GB of VRAM. Aside from current gen higher-end cards, which are still ludicrously priced, the 3090 has 24GB but costs a fortune even used. The 5060 Ti hit the sweet spot for my budget. I wasn't considering AMD cards but the software ecosystem around them is maturing to the point where I think I'd consider that now as well if I were to do it all over again.

The difference with 16GB VRAM was night and day. Doubling my GPU's VRAM increased my AI VRAM budget from ~5GB usable to ~13GB usable (I still had to leave room for the display and system overhead) and meant that I could run significantly larger models without spilling into system RAM. Models that were previously unusable because of VRAM spillover now ran at full speed. The TTFT for models that fit cleanly in VRAM dropped to seconds instead of tens of seconds. And most importantly the models that can fit into 16GB, especially with newer quantization strategies, start feeling meaningfully close to cloud model quality for a lot more tasks than those that fit into 8GB VRAM. That's not to say that you can't get anything useful done with 8GB VRAM, you absolutely can -- you just need to adjust expectations and break down work into smaller pieces.
...
Quantization is one of the most important decisions in model selection, and it's the reason we're able to run larger models on consumer hardware at all. Quantization is the process of reducing a model's precision from 16-bit floats to 4-bit integers (or 5-bit, or 6-bit, etc...). It shrinks the model's memory footprint significantly, but at some cost to quality. The key is finding the right balance - small enough to fit and run efficiently while preserving quality enough to still be reliable.
...
This is where I left the nicely curated world of the Ollama model library and entered the wild west of Hugging Face. If you stick with this local LLM hobby, you won't be able to avoid Hugging Face. It's the de facto hub for open source AI everything - models, datasets, apps, and so on. And I need to tell you right up front that the Hugging Face user experience is awful. It's a website that accreted its functionality over years without any overarching design plan. It caters to ML researchers and clearly not enough UX expertise has been present. It's incredibly hard to navigate and find what you're looking for, and the information is often presented in a nearly-incoherent mess of spaghetti. Having said that, it's also indispensible - from Hugging Face I can download any GGUF quantization without the Ollama gatekeeping and compare a model at Q4 vs Q5 vs Q6 vs UD-Q4 vs UD-IQ3_S side by side, which will reveal meaningful differences.

For CLI fans, there's a huggingface CLI that helps reduce the need to wade through the website somewhat, which makes using HF slightly less frustrating for me.

## Results by Use Case

Pretty early on in testing I started gravitating towards the Qwen models as they just felt more polished to me than Gemma and the others. I like the default tone of the Qwen models a little more, and on a parameter basis felt more effective than their equivalents from other model families.

### Chat: Surprisingly, Smaller Models Were Fine

I expected that for chat I'd need a model with enough parameters to handle the nuances of conversation, but it turned out that **Qwen 3.5 9B** was entirely adequate for chat. It handled follow-up questions well, maintained context across a reasonable conversation window, and didn't hallucinate as often as I expected.

Even smaller models — the 3B and 5B variants — were usable for basic chat. They weren't as good at handling complex follow-ups, but for quick questions and casual conversation, they felt fine. This matters because it means I don't *always* need the biggest model and can have a specific model set up for quick chats.
...
For document processing, again **Qwen 3.5 9B** was the best balance of quality and performance. For most documents it would be indistinguishable from the Qwen3 14B model in terms of accuracy. For trickier documents with unusual formatting or mixed content, the 14B pulled ahead somewhat, but at a significant performance cost.
...
**Qwen 3.5 9B** was surprisingly decent at code analysis, even though it wasn't specifically trained for coding. If you're just working on cleanup of an existing codebase, this might be a totally fine choice that will run more responsively.
...
I tested tool use (models calling external tools like a web search API) across several models, as this is essential for agentic coding. I started with Claude Code and then switched to Opencode as the agentic AI client. The results were inconsistent. Some models supported tool use but broke as session length grew. Others kind of worked but produced malformed output. A few didn't support it at all.
...
This means that **a model that fits cleanly in VRAM at the start of a conversation can spill into system RAM partway through**, and you won't know it until performance suddenly seriously degrades.

This is another area where MoE models are helpful. The KV cache for an MoE model is smaller relative to total parameter count since only the active experts' parameters need KV cache entries. This is one reason why Qwen 3.6-35B-A3B runs so efficiently for coding — its KV cache footprint is more like a 7B dense model, not a 35B model. That's a significant advantage for VRAM-constrained setups, and it's critical for agentic coding, where I find I need a minimum of 128K context window to be effective. This would be impossible on my hardware without the MoE optimization.
...
So if you're reading this and thinking about choosing models for your own setup, here's a framework you can use:

### Step 1: Know Your VRAM Budget

Subtract the VRAM your display and system apps use. That's your starting point. Then decide how much you want to leave as overhead. The remainder is your model + KV cache budget. Leave more than you think aside for KV cache, you'll need it! Now you know what size (on disk, not parameters) model you should target.
...
Use something like paperless-gpt for document processing — it's a well-defined task with clear right/wrong answers. If a model struggles there, it's unlikely to be great for anything. Use it as a sniff test for a new model. It's a bit easier to get an objective comparison with this kind of test than it is with a generic chat session.
...
1. **VRAM is still king.** Doubling from 8GB to 16GB changed everything. The model that fits cleanly in VRAM will feel better than a larger model that spills into RAM. Even the 5060 ti, which has relatively modest memory bandwidth relative to higher-end cards, made a huge difference for me.
...
4. **Quantization matters — and is evolving fast.** Uunsloth's UD quantizations and similar newer schemes are changing the math. A model that wouldn't have worked at Q4 might work great at UD-IQ3_S.
5. **Context windows are a large cost.** The KV cache eats VRAM faster than you'd expect. Plan for it. MoE models help here since their KV cache is smaller relative to total parameter count.
...
I've also switched inference servers again, this time from LM Studio to llama-server. llama-server gives me the ultimate configuration control, and there are a number of tweaks you can make at startup to squeeze out some more performance. With local LLMs, every extra token/sec you can achieve is useful.
